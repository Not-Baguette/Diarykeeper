...
def diary_root():
    ...
    def about():
        """
        Display a window providing general information about the program
        """
        root_about = tk.Toplevel(root)
        root_about.title("About")
        root_about.resizable(False, False)

        font_name = "Arial"
        about_label = tk.Label(root_about, text="This is a diary app made by Not-baguette in Python 3.10.\n It uses "
                                                "SHA-256 hashing for keeping your password safe \n and a modified "
                                                "Twofish encryption algorithm to encrypt your diary. \n"
                                                "Completely offline. We value you and keep you away from worries.",
                               font=(font_name, 12))
        about_label.grid(row=0, column=0, padx=10, pady=5)

    def support():
        """
        Send the user to the isssue page
        """
        webbrowser.open_new_tab("https://github.com/Not-Baguette/Diarykeeper/issues")
